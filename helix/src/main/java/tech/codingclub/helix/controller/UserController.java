...
public class UserController extends BaseController {
    ...
    public
    @ResponseBody
    List<TweetUI> fetchTweet(@PathVariable ("id") Long tweetId, HttpServletRequest request, HttpServletResponse response) {

        Condition condition = tech.codingclub.helix.tables.Tweet.TWEET.ID.lessThan(tweetId);
        ...
        for(Tweet tweet : data){
            ...
            TweetUI tweetUI = new TweetUI(member, tweet);
    ...
    String followMember(@RequestBody Long memberId, HttpServletRequest request, HttpServletResponse response) {
        ...
        //want to get connected with currentUser
    ...
    String unFollowMember(@RequestBody Long memberId, HttpServletRequest request, HttpServletResponse response) {
        ...
        if(currentUserId != null && memberId != null && !currentUserId.equals(memberId)) {
            //Delete a relation between currentUserId and memberId
    ...
    public String welcome(ModelMap modelMap, HttpServletResponse response, HttpServletRequest request) {
        ...
        //for recommendation
        //Don't want to recommend himself
        Condition conditionForSelf = tech.codingclub.helix.tables.Member.MEMBER.ID.notEqual(member.id);
        List<Member> members = (List<Member>) GenericDB.getRows(tech.codingclub.helix.tables.Member.MEMBER, Member.class, conditionForSelf, 10, tech.codingclub.helix.tables.Member.MEMBER.ID.desc());
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/followingMe")
    public String following(ModelMap modelMap, HttpServletResponse response, HttpServletRequest request) {
        Long currentMemberId = ControllerUtils.getUserId(request);

        //access only when he is logged in!

        Condition condition = tech.codingclub.helix.tables.Follower.FOLLOWER.FOLLOWING_ID.eq(currentMemberId);
        List<Long> followingIDs = new GenericDB<Long>().getColumnRows(tech.codingclub.helix.tables.Follower.FOLLOWER.USER_ID, tech.codingclub.helix.tables.Follower.FOLLOWER, Long.class, condition, 100);

        Condition selectMemberCondition = tech.codingclub.helix.tables.Member.MEMBER.ID.in(followingIDs);
        List<Member> followedMembers = (List<Member>) GenericDB.getRows(tech.codingclub.helix.tables.Member.MEMBER, Member.class, selectMemberCondition, 10, tech.codingclub.helix.tables.Member.MEMBER.ID.desc());


        modelMap.addAttribute("FOLLOWINGME", followedMembers);
